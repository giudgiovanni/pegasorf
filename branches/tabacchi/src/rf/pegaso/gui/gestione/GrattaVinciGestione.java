...
import rf.pegaso.db.model.GrattaEVinciModel;
 ...
 *
 */
public class GrattaVinciGestione extends JFrame {
	...
	class MyMouseAdapter extends MouseAdapter{
		...
		public void mouseClicked(MouseEvent e) {
			if(e.getSource()==tblGrattaVinci && e.getClickCount()==2){
				modificaArticolo();
			}
		}
	...
	private JTable tblGrattaVinci = null;
	...
	private TableRowSorter<GrattaEVinciModel> sorter;  //  @jve:decl-index=0:
	...
	public void duplicaArticolo() {
		if (tblGrattaVinci.getSelectedRow() <= -1) {
		...
		int riga = tblGrattaVinci.getSelectedRow();
		int idArticolo = ((Long) tblGrattaVinci.getValueAt(riga, 0)).intValue();
		...
		try {
			...
			GrattaEVinciAddMod mod = new GrattaEVinciAddMod((JFrame)this, idNewArticolo,ArticoliAddMod.MOD);
			mod.setVisible(true);
	...
	private void eliminaArticolo() {
		...
		if (scelta != JOptionPane.YES_OPTION)
			return;
		int nSel = tblGrattaVinci.getSelectedRowCount();
		int riga = 0;
		int righe[] = null;
		if (nSel == 1) {
			riga = tblGrattaVinci.getSelectedRow();
			int idArticolo = ((Long) tblGrattaVinci.getValueAt(riga, 0))
					.intValue();
			Articolo r = new Articolo();
			try {
	...
	private JScrollPane getJScrollPane() {
		if (jScrollPane == null) {
			try {
				...
				jScrollPane.setViewportView(getTblGrattaVinci()); // Generated
			} catch (java.lang.Throwable e) {
				// TODO: Something
			}
		}
	 ...
	 */
	private JTable getTblGrattaVinci() {
		if (tblGrattaVinci == null) {
			try {
				GrattaEVinciModel modello = new GrattaEVinciModel(dbm);
				dbm.addDBStateChange(modello);
				sorter=new TableRowSorter<GrattaEVinciModel>(modello);
				tblGrattaVinci = new JTable(modello);
				tblGrattaVinci.setRowSorter(sorter);
				// tblArticoli.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);

				tblGrattaVinci.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);
				//tblArticoli.packAll();
				tblGrattaVinci.getTableHeader().setReorderingAllowed(false);


//				 impostiamo le varie colonne
				TableColumn col = tblGrattaVinci.getColumnModel().getColumn(0);
				col.setMinWidth(0);
				col.setMaxWidth(0);
				col.setPreferredWidth(0);

				col = tblGrattaVinci.getColumn("codice");
				...
				col = tblGrattaVinci.getColumn("descrizione");
				...
				col = tblGrattaVinci.getColumn("aams");
				...
				col = tblGrattaVinci.getColumn("prezzo_acquisto");
				...
				col = tblGrattaVinci.getColumn("prezzo_listino");
				...
				col = tblGrattaVinci.getColumn("fornitore");
	...
	private void initialize() {
		...
		this.setTitle("Gestione Gratta e Vinci");
		...
		tblGrattaVinci.addMouseListener(myMouseadapter);
	}
	...
	private void modificaArticolo() {
		...
		int riga = tblGrattaVinci.getSelectedRow();
		int idArticolo = ((Long) tblGrattaVinci.getValueAt(riga, 0)).intValue();
		GrattaEVinciAddMod mod = new GrattaEVinciAddMod(this,idArticolo,ArticoliAddMod.MOD);
		mod.setVisible(true);
		tblGrattaVinci.setRowSelectionInterval(riga, riga);
	...
	private void nuovoArticolo() {
		...
		// del nuovo reparto
		GrattaEVinciAddMod add = new GrattaEVinciAddMod(this, ArticoliAddMod.ADD);
		add.setVisible(true);
...
//    }
	...
	private void newFilter() {
		RowFilter<GrattaEVinciModel, Object> rf = null;
        RowFilter<GrattaEVinciModel, Object> rf1 = null;
        RowFilter<GrattaEVinciModel, Object> rf2 = null;
        RowFilter<GrattaEVinciModel, Object> rf3 = null;
        ...
        try {
            ...
            ArrayList<RowFilter<GrattaEVinciModel,Object>> filters = new ArrayList<RowFilter<GrattaEVinciModel,Object>>(2);
            filters.add(rf1);
            filters.add(rf2);
            filters.add(rf3);
            rf = RowFilter.andFilter(filters);
