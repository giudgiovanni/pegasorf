 ...
 * 
 */
public class RepartiMod extends JDialog {
	 ...
	 * @param owner
	 */
	public RepartiMod(JDialog owner, DBManager dbm, int idReparto) {
		super(owner, true);
		this.dbm = dbm;
	...
	private JPanel getPnlCentrale() {
		if (pnlCentrale == null) {
			try {
				...
				jLabel1.setBounds(new Rectangle(11, 34, 111, 16)); // Generated
				...
				pnlCentrale.setBorder(BorderFactory.createTitledBorder(
						...
						"Dati Categoria", TitledBorder.DEFAULT_JUSTIFICATION,
	...
	private void initialize() {
		...
		this.setTitle("Modifica Categoria"); // Generated
		this.setContentPane(getJContentPane());
		UtilGUI.centraDialog(this);
		Reparto r = new Reparto();
		try {
			r.caricaDati(this.idReparto);
	...
	private void modifica() {
		int scelta = JOptionPane.showConfirmDialog(this,
				"Sei sicuro di voler\neffettuare le modifiche?", "AVVISO",
				JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
		if (scelta != JOptionPane.YES_OPTION)
			return;
		Reparto r = new Reparto();
		r.setIdReparto(this.idReparto);
		r.setDescrizione(this.txtDescrizione.getText());
		r.setNome(this.txtNome.getText());
		try {
			r.updateReparto();
		} catch (IDNonValido e) {
			JOptionPane.showMessageDialog(this, "Valore idReprto errato",
					"ERRORE", JOptionPane.ERROR_MESSAGE);
			e.printStackTrace();
		}
		dispose();
