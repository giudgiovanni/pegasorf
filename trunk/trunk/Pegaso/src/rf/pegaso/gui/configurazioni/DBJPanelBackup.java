...
public class DBJPanelBackup extends JPanel {
	...
	protected void creaBakup() {
		try {
			...
			JOptionPane.showMessageDialog(getParent(),
					"Errore nel file di configurazione", "ERRORE",
	...
	private void caricaLista() {
		try {
			...
			// ritardiamo di 1sec per far si che prima vengano calcellati i file
			// del tutto
			// altrimenti visualizza alcuni dati che poi effettivamente ancano.
			Thread.sleep(1000);
			String nf[] = UtilityDBManager.getSingleInstance()
					.elencoFileBackup();
			for (String tmp : nf) {
	...
	protected void restoreBackup() {
		try {
			...
			UtilityDBManager.getSingleInstance().restoreBackup(
					(String) lstElencoBackup.getSelectedValue(), listModel);
		} catch (FileNotFoundException e) {
			JOptionPane.showMessageDialog(getParent(),
					"Errore nel file di configurazione", "ERRORE",
					JOptionPane.ERROR_MESSAGE);
