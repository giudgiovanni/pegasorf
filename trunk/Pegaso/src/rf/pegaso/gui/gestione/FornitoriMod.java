...
public class FornitoriMod extends JDialog {
	 ...
	 * @param owner
	 */
	public FornitoriMod(JDialog owner, DBManager dbm, int idCliente) {
		super(owner, true);
		this.dbm = dbm;
		this.idFornitore = idCliente;
		initialize();
	}
	...
	private JPanel getPnlDatiPersonali() {
		if (pnlDatiPersonali == null) {
			try {
				...
				lblNome = new JLabel();
				lblNome.setText("Ragione Sociale"); // Generated
				lblNome.setBounds(new Rectangle(6, 33, 91, 16)); // Generated
				pnlDatiPersonali = new JPanel();
				pnlDatiPersonali.setLayout(null); // Generated
				pnlDatiPersonali.setBorder(BorderFactory
						.createBevelBorder(BevelBorder.RAISED)); // Generated
				pnlDatiPersonali.add(getTxtRagSociale(), null); // Generated
	...
	private JTextField getTxtRagSociale() {
		if (txtRagSociale == null) {
			try {
				...
				txtRagSociale.setPreferredSize(new Dimension(140, 20)); // Generated
				txtRagSociale.setBounds(new Rectangle(6, 49, 366, 20)); // Generated
	 ...
	 * @param c
	 */
	private void impostaCampi(Fornitore c) {
		txtRagSociale.setText(c.getNome());
		txtPiva.setText(c.getPiva());
		txtCodFisc.setText(c.getCodfisc());
		txtVia.setText(c.getVia());
	...
	private void initialize() {
		...
		this.setTitle("Modifica Fornitori"); // Generated
	...
	private void modifica() {
		if (idFornitore <= 0)
			JOptionPane.showMessageDialog(this, "Codice idFornitore errato",
					"ERRORE", JOptionPane.ERROR_MESSAGE);
		int scelta = JOptionPane.showConfirmDialog(this,
				"Sei sicuro di voler\nmodificare il fornitore?", "AVVISO",
				JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE);
		if (scelta != JOptionPane.YES_OPTION)
			return;
		Fornitore c = new Fornitore();
		c.setIdFornitore(this.idFornitore);
		c.setNome(txtRagSociale.getText());
